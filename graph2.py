...
# Define the number of goals (you may adjust these as needed)
num_goals_values = [1, 2, 3, 5, 7, 10]  # Updated values

# Loop through number of goals
for num_goals in num_goals_values:
    # Define the paths to the CSV files for Dijkstra and A*
    dijkstra_csv_path = os.path.join(results_directory, "Dijkstra/num_goals", str(num_goals) + ".csv")
    a_star_csv_path = os.path.join(results_directory, "A*/num_goals", str(num_goals) + ".csv")
...
plt.errorbar(num_goals_values, [((d_min + d_max) / 2) for d_min, d_max in dijkstra_peak_memory],
             yerr=[(d_max - d_min) / 2 for d_min, d_max in dijkstra_peak_memory], label='Dijkstra', marker='o', capsize=5)
plt.errorbar(num_goals_values, [((a_min + a_max) / 2) for a_min, a_max in a_star_peak_memory],
...
plt.title('Peak Memory Usage vs. Number of Goals')
